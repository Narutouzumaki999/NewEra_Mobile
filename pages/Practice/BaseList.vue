<template>
	<scroll-view scroll-y :style="{height: '100%'}" class="swiperItem" @scrolltolower="onreachBottom">
		<view class="newsItem" v-for="(item, index) in a_showList" :key="index" @click="onPreviewBase(item)">
...
<script>
	export default {
		...
		methods: {
			onPreviewBase(item) {
				uni.navigateTo({
					url: `/pages/Practice/BasePreview/BasePreview?id=${item.id}`
				})
			},
